          ...
          child: ListView(
            children: <Widget>[
              Padding(
                ...
                child: Visibility(visible: widget.status == changePass,child: Text('Enter Your old password below')),
              ),
              Visibility(visible: widget.status == changePass ,child: TextFormField(
                controller: _oldPasswordController,
                validator: widget.status == changePass ? (value){
                  if(value.isEmpty){
                ...
                decoration: InputDecoration(
                    ...
                    suffixIcon: IconButton(icon: _showOldPass ? Icon(Icons.visibility_off) : Icon(Icons.visibility), onPressed: (){
                      setState(() {
                        _showOldPass ^= true;
                      });
                    })
              ...
              Padding(
                ...
                child: Text('Enter and repeat your new password in the inputs below',style: TextStyle(fontSize: 18,color: Colors.teal)),
              ),
              TextFormField(
                ...
                decoration: InputDecoration(
                    ...
                    suffixIcon: IconButton(icon: _showNewPass ? Icon(Icons.visibility_off) : Icon(Icons.visibility), onPressed: (){
                      setState(() {
                        _showNewPass ^= true;
                      });
                    })
              ...
              Padding(padding: EdgeInsets.symmetric(vertical: 10.0),child: TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: IconButton(icon: _showRptPass ? Icon(Icons.visibility_off) : Icon(Icons.visibility), onPressed: (){
                    setState(() {
                      _showRptPass ^= true;
                    });
                  })
              ...
              CupertinoButton(color: Colors.blueGrey,child: Text('Change password'), onPressed: () async{
                if(_formKey.currentState.validate()){
                  ...
                  String encodedNewPass = base64.encode(utf8.encode(newPassInput));
                  int userId = _thisUser.id;
                  if(widget.status == changePass){
                    String encodedPass = base64.encode(utf8.encode(oldPass));
                    ...
                    HttpClientResponse rsp = await getRequestObject(url+'mobileuser/checkpass/$userId?'+uqp.toString(), get,dialog: dial);
                    if(rsp != null){
                      rsp.transform(utf8.decoder).listen((data) async{
                        var jsonResponse = json.decode(data);
                        bool correctPass = jsonResponse['correctPass'];
                        if(correctPass){
                          dial.update(message: 'Updating new Password ... ');
                          String jsonData = json.encode(User(id: userId,password: encodedNewPass));
                          HttpClientResponse hcr = await getRequestObject(url+'mobileuser/update', put,body: jsonData,dialog: dial);
                          if(hcr != null){
                            hcr.transform(utf8.decoder).listen((data){
                              var jsonResponse = json.decode(data);
                              bool updated = jsonResponse['updated'];
                              if(updated){
                                Fluttertoast.showToast(msg: 'Your password has been updated successfully');
                    ...
                    String jsonData = json.encode(User(id: userId,password: encodedNewPass));
                    String url = await getBaseUrl();
                    HttpClientResponse res = await getRequestObject(url+'mobileuser/update', put,body: jsonData,dialog: dial);
                    if(res != null){
